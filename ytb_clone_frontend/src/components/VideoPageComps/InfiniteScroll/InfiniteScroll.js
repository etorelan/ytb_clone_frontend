...
function InfiniteScroll({
	...
	const { sidebarVideos, setSidebarVideos, videoInfo } =
	...
	const loadMoreComments = async (videoId, lastIndex) => {
		if (videoId) {
			const videoSnap = await getDoc(
				doc(db, "videos", videoId.toString())
			);
			if (videoSnap.exists()) {
				try {
					const videoData = videoSnap.data();
					let newObj = videoData.comments.slice(
						lastIndex,
						lastIndex + LOAD_COMMENTS_PER_PAGE
					);
					return newObj;
				} catch (error) {
	...
	const displayMoreComments = async () => {
		...
		if ((page * DISPLAY_COMMENTS_PER_PAGE) % LOAD_COMMENTS_PER_PAGE == 0) {
			let nextItems = await loadMoreComments(videoId, endIndex);
			let placeholder = stateItems.concat(nextItems);
			setStateItems(placeholder);
		}
		const newDisplayedItems = [
			...displayedItems,
			...placeholder.slice(startIndex, endIndex),
		];
	...
	return (
		<div>
			{displayedItems.map((item, index) => (
				<div key={index}>
					{React.createElement(Component, { Id: item })}
				</div>
			))}
		</div>
	);
